    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        # 检测 TensorRT 版本
            ...
            if is_trt10:
                ...
                for i in range(model.num_io_tensors):
                    ...
                    if model.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
                        if -1 in tuple(model.get_tensor_shape(name)):
                            dynamic = True
                            context.set_tensor_shape(name, tuple(model.get_tensor_profile_shape(name, 0, 2)))
                        if dtype == np.float16:
                            fp16 = True
                    else:  # OUTPUT
                ...
                binding_addrs = {name: bindings[name].ptr for name in bindings}
                batch_size = bindings[list(bindings.keys())[0]].shape[0]
            else:
                # TensorRT 7-8 旧 API
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            ...
            if self.dynamic and im.shape != self.bindings['images'].shape:
                if self.is_trt10:
                    # TensorRT 10.x 新 API
                    self.context.set_tensor_shape('images', im.shape)
            ...
            if self.is_trt10:
                ...
                # 设置所有输出张量的地址
                for name in self.output_names:
                    self.context.set_tensor_address(name, self.bindings[name].ptr)
